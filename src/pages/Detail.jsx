...
export default function Detail() {
  ...
  if (loading) return (
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f9fafb' }}>
      ...
      <div style={{textAlign:'center'}}><div style={{width:'36px',height:'36px',borderRadius:'50%',border:'3px solid #f3f4f6',borderTop:'3px solid #f97316',animation:'sg-spin 0.8s linear infinite',margin:'0 auto 14px'}}/><p style={{fontSize:'13px',color:'#9ca3af',fontWeight:500}}>Loading UV detail...</p></div>
    </div>
  )

  if (error || !data) return (
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f9fafb' }}>
      <p style={{fontSize:'14px',color:'#ef4444',textAlign:'center',padding:'0 24px'}}>{error ?? 'Unable to load page data.'}</p>
    </div>
  )
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#ffffff' }}>
      <style>{`
        ...
        .detail-left { border-right: 1px solid #f3f4f6; }
        .detail-right { display: flex; flex-direction: column; }
        .detail-right-section { border-bottom: 1px solid #f3f4f6; padding: 32px 32px; }
        .detail-right-section:last-child { border-bottom: none; }
        @media (max-width: 960px) {
          .detail-layout { grid-template-columns: 1fr !important; }
          .detail-left { border-right: none !important; border-bottom: 1px solid #f3f4f6; }
          .detail-right-section { padding: 28px 24px !important; }
        }
        @media (max-width: 768px) {
          .detail-hero-inner { grid-template-columns: 1fr !important; }
        }
      `}</style>

      {/* ── Hero ── */}
      <div style={{ background: theme.heroGrad, padding: 'clamp(24px, 4vw, 40px) clamp(20px, 4vw, 48px) clamp(28px, 5vw, 48px)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth:'1280px', margin:'0 auto', position:'relative', zIndex:1 }}>
          <p style={{ color:'rgba(255,255,255,0.72)', fontSize:'13px', marginBottom:'14px' }}>{data.dateLabel}</p>
          <div className="detail-hero-inner" style={{ display:'grid', gridTemplateColumns:'auto 1fr', gap:'36px', alignItems:'center' }}>
            {/* UV card */}
            ...
            <div>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:'8px'}}>
                {[{icon:'🔺',text:`Peak ${data.peakUV} at ${data.peakTime}`},{icon:'🌅',text:`Lowest ${data.lowestUV} at ${data.lowestTime}`},{icon:'🌡️',text:`${data.temperature}° · ${weather.desc}`}].map((chip,i)=>(
                  <span key={i} style={{display:'inline-flex',alignItems:'center',gap:'6px',fontSize:'12px',fontWeight:600,color:'#fff',background:'rgba(255,255,255,0.12)',border:'1px solid rgba(255,255,255,0.18)',padding:'5px 10px',borderRadius:'999px'}}>
      ...
      <div style={{ maxWidth:'1280px', margin:'0 auto', padding:'0 clamp(20px, 4vw, 48px)' }}>
        <div className="detail-layout">
          ...
          {/* LEFT: UV Trend */}
          <div className="detail-left" style={{ padding:'32px 32px 32px 0' }}>
            <h2 style={{fontSize:'22px',fontWeight:700,color:'#1c1917',marginBottom:'4px'}}>Today's UV Trend</h2>
            <p style={{fontSize:'13px',color:'#57534e',marginBottom:'16px'}}>Peak: <strong>{data.peakUV}</strong> ({data.peakTime}) · Lowest: <strong>{data.lowestUV}</strong> ({data.lowestTime})</p>
            <div style={{borderRadius:'12px',overflow:'hidden',background:'linear-gradient(180deg,#23252d 0%,#2c2f37 100%)',boxShadow:'0 8px 24px rgba(0,0,0,0.16)',border:'1px solid #3f3f46'}}>
          ...
          <div className="detail-right">
            ...
            <div className="detail-right-section">
              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'16px'}}>
                ...
                <h2 style={{fontSize:'18px',fontWeight:700,color:'#1c1917',margin:0}}>Protection Advice</h2>
              </div>
              <div style={{borderRadius:'14px',background:theme.adviceBg,border:`1px solid ${theme.adviceBorder}`,padding:'16px 18px'}}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'10px',flexWrap:'wrap',marginBottom:'10px'}}>
                  <h3 style={{fontSize:'16px',fontWeight:700,color:theme.adviceText,margin:0}}>{pa.title}</h3>
                ...
                <p style={{fontSize:'13px',color:theme.adviceText,lineHeight:1.7,margin:'0 0 12px'}}>{pa.description}</p>
                {pa.spfCase === 'none' && (
                  <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                    {pa.tips.map((tip,i)=><div key={i} style={{background:'rgba(255,255,255,0.5)',border:`1px solid ${theme.adviceBorder}`,borderRadius:'8px',padding:'8px 12px',fontSize:'12px',color:theme.adviceText,lineHeight:1.5}}>{tip}</div>)}
                  </div>
                )}
                {pa.spfCase !== 'none' && pa.recommendedAmount.length > 0 && (
                  <div>
                    <p style={{fontSize:'13px',fontWeight:600,color:theme.adviceText,marginBottom:'6px'}}>Recommended amount</p>
                    <ul style={{margin:0,paddingLeft:'18px',color:theme.adviceText}}>
                      {pa.recommendedAmount.map((item,i)=><li key={i} style={{fontSize:'12px',lineHeight:1.8}}>{item}</li>)}
                    </ul>
                    <p style={{fontSize:'10px',color:theme.adviceText,opacity:0.6,marginTop:'10px',lineHeight:1.5}}>* Amounts estimated based on the Mosteller formula.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="detail-right-section">
              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'16px'}}>
                ...
                <h2 style={{fontSize:'18px',fontWeight:700,color:'#1c1917',margin:0}}>Suggested Outfit</h2>
              </div>
              <div style={{borderRadius:'14px',background:'#fff1f2',border:'1px solid #fecdd3',padding:'16px 18px'}}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'10px',flexWrap:'wrap',marginBottom:'10px'}}>
                  <h3 style={{fontSize:'16px',fontWeight:700,color:'#9f1239',margin:0}}>{data.outfitAdvice.title}</h3>
                ...
                <p style={{fontSize:'13px',color:'#881337',lineHeight:1.75,margin:0}}>{data.outfitAdvice.description}</p>
              </div>
            </div>

...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('https://uv-level-monitor-anb3fvckcsfcf4a3.australiaeast-01.azurewebsites.net/update_status', {
      ...
      .then(res => {
        const parsed = {
          cityName, temperature: res.temperature ?? 0,
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '13px', color: '#9ca3af', fontWeight: 500 }}>Loading UV data...</p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9fafb', position: 'relative' }}>
      ...
      {showUVAlert && uvAlertContent && (
        <div style={{ position: 'fixed', top: '80px', left: '50%', transform: 'translateX(-50%)', zIndex: 1000, width: 'min(480px, 92vw)', background: '#fff', borderRadius: '20px', boxShadow: '0 12px 40px rgba(0,0,0,0.20)', border: `2px solid ${uvAlertContent.borderColor}`, padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', animation: 'slideDown 0.3s ease' }}>
          ...
          <button onClick={() => setShowUVAlert(false)} style={{ background: `linear-gradient(135deg, ${uvAlertContent.borderColor}, ${uvAlertContent.borderColor}cc)`, color: '#fff', border: 'none', borderRadius: '12px', padding: '11px', fontWeight: 600, fontSize: '14px', cursor: 'pointer', width: '100%' }}>✓ Got it</button>
        </div>
      )}
